...
//DRIVE PID VARIABLES//
const float kL = 0.05; //Kp value for usercontrol integrated PID
const float kR = 0.05;
const float kG = 0.03;
#define MAXL 1.00 //max fraction of full power output while recording (so in auton it can compensate)
#define MAXR 1.00
...
task record()
{
		//Record
			...
			while(true) {//Loop
				...
				//Drive
				L[t/dTplayback]= fabs(getMotorEncoder(BL)) + ((1/kL)*motor[BL]); //Left, stores target every time
				R[t/dTplayback]= fabs(getMotorEncoder(BR)) + ((1/kR)*motor[BR]); //Right
				//Gyro
				...
				//Break
				if(vexRT[Btn8U]==1||vexRT[Btn8UXmtr2]==1||t>57700){break;}
...
{ //Tasks
	...
	while (true)
	{
		...
		//Drive
		motor[BL]=motor[FL]=vexRT[Ch3];
		motor[BR]=motor[FR]=vexRT[Ch2];
