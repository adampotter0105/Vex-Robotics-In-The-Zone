...
int TError = 1.0; //0%
int turnError=0;
/*int L[600];//20 samples/s *60 seconds
int R[600];
int T[250];
int B[250];
int In[150];
...
//Random Functions
int maxLimit(int max, float input){if(fabs(input)>max){return max*sgn(input);}else{return input;}}//overcomplicated rounding function
...
displayLCDPos(0, 0);
 ...
 displayLCDPos(0, 7);
 displayNextLCDNumber(SensorValue[yaw], 4);

	if(nLCDButtons ==2){	//Calibrate Gyro
...
task PID()  //currently only using two PID loops
...
void begin(){
	intakeon=1;
		motor[goalL]=motor[goalR]=-127;//Goal
		motor[intake]=motor[intake2]=-127;//Intake
		delay(550);
		PIDon=1; target=barHI[0];//Bar
		motor[BL]=motor[BR]=motor[FL]=motor[FR]=100;//Drive
		motor[intake]=motor[intake2]=-30;//Intake
		delay(620);
		PIDon=0; motor[barL]=motor[barR]=-127;//Bar
		delay(200); //forward to goal
		motor[goalL]=motor[goalR]=0;//goal stops
		motor[BL]=motor[BR]=motor[FL]=motor[FR]=-20; //Drive
		delay(250); motor[BL]=motor[BR]=motor[FL]=motor[FR]=-15;//Drive
		delay(260); motor[barL]=motor[barR]=0; //Bar
		motor[intake]=motor[intake2]=127;//Intake
		delay(250);
		motor[intake]=motor[intake2]=0;//Intake
		PIDon=1; target=barHI[0];//Bar
		begintoggle = 0;
}
...
{  while(true){
		...
		if(vexRT[Btn7U]==1||vexRT[Btn7UXmtr2]==1){ // if buttons pressed
			...
			while(true) {//Loop
				...
				//Drive
				L[t/dTplayback]= ((1/kL)*motor[BL]); //Left, stores target every time
				R[t/dTplayback]=  ((1/kR)*motor[BR]); //Right
				//Gyro
...
{ stopTask(intaketoggle);
	...
	while(true)
	{
		...
		motor[BL]=motor[FL]=(int)(kL*TError*(L[t/dTplayback]));//L   using proportional and same targets as drive

		motor[BR]=motor[FR]=(int)(kR*TError*(R[t/dTplayback]));
		//Bars and Intake
...
{ //Tasks
	...
	{   //DELAY
		...
		//Drive
		motor[BL]=motor[FL]=vexRT[Ch3];/*(int)maxLimit(127*maxL,(0.093*pow(fabs(vexRT[Ch3]),1.5))*sgn(vexRT[Ch3]));*/
		motor[BR]=motor[FR]=vexRT[Ch2];/*(int)maxLimit(127*maxR,(0.093*pow(fabs(vexRT[Ch2]),1.5))*sgn(vexRT[Ch2]));*/
		...
		if(vexRT[Btn7U]==1){stopTask(intaketoggle); begin(); startTask(intaketoggle);}
